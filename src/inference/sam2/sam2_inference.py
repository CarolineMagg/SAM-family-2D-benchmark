...
from src.inference.sam2.sam2_utils import create_sam2_predictor, all_sam2_model_type_options
...
def run_inference_sam2(json_file: Union[Path, str], output_folder: Union[Path, str], prompt_type: list[str],
    ...
    predictor = create_sam2_predictor(device, model_type, video_predictor=False)

    print("")
    print(f"SAM2 inference with {model_type}")
    ...
    # meta data json
    all_meta_data = {"model": "SAM2", "model_type": model_type,
        ...
        for slice_idx in tqdm(range(len(input_image)), desc="Slices"):
            ...
            if str(slice_idx) in all_slices_with_prompts:
                ...
                for idx, label in enumerate(labels_values):
                    prompts_2d: dict[str, list] = data_prompt[file_id][str(label)]["2d_prompts"]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SAM2 inference for combination of prompts from json file")
    ...
    parser.add_argument("--model_type", default="sam2_new_hiera_base_plus",
                        choices=all_sam2_model_type_options)
    args = parser.parse_args()

    run_inference_sam2(args.json_file,
                       args.output_folder,
                       args.prompt_type,
                       args.number_prompts,
